...
---
## P-2026-07-26 — Video editor "Remove Watermark → Apply" (Blur mode) does nothing / "Watermark removal failed"
- **Symptom (Henry):** on https://andiostudio.com/dashboard/video-editor, Remove Watermark with default **Mode: Blur** + any corner (BR/BL/TR/TL) failed — no output, error message.
- **Root cause:** `delogo` reconstructs the covered area by interpolating from the pixels immediately AROUND the box, so it needs a border of real pixels on all four sides. Every `CORNER_PRESET` ran to the frame edge (`br` → x+w=W, `tl` → x=0 & y=0, etc.) and `clampBox` allowed a box flush against the edge (`bx∈[0,W-2]`, `bw` up to `W-bx`). A delogo box touching ANY edge makes ffmpeg exit **-22 "Invalid argument"** and write **no output** → `runFFmpeg` rejects → 500 → "Watermark removal failed". Only Blur hit it; Cover uses `drawbox`, which has no edge constraint (so cover "worked", blur didn't).
- **Fix (PR #256, `app/api/editor/dewatermark/route.ts`):** `clampBox` now keeps an even **2px margin inside every edge** (`EDGE_MARGIN`). Invisible shrink for the cover box; the required sampling border for delogo. Verified: `br`/`tl` edge-touch boxes reproduce `-22`+no-output; 2px-inset box clean; all 4 presets across 1920×1080 / 1080×1920 / 1280×720 sit inside; full re-encode pipeline produces valid output; `tsc` clean.
- **Prevention:** any ffmpeg `delogo` box MUST sit strictly inside the frame with a ≥1px (even → 2px) border on ALL sides — never derive a delogo box straight from a corner-anchored fractional preset without insetting. A filter that "does nothing / no output" with rc≠0 and `-22` = geometry outside the frame; check box-vs-edge first. (Class-level: mirrored to global error_log.md.)
...
> "free mode did excellently just subsitle text are big not too big ok but over flowing on screen"
> "hybrid is somehting else no action imae - image are all pos and smiling not not action scene"

**Two distinct bugs, same report:**

### A) Subtitle PNG always rendered at 1920×1080 (landscape)
`generateSubtitlePng()` hard-coded `W=1920, H=1080`. On a 9:16 video (1080×1920), the FFmpeg overlay placed the PNG at native size — text rendered centered at PNG midpoint x=960 lands OFF the 1080-wide video, and the right ~840px of the PNG fell off-frame. User saw "subtitle overflowing".

**Fix:** Added optional `aspectRatio` param to `generateSubtitlePng()`; canvas now sizes to match (`9:16 → 1080×1920`, `1:1 → 1080×1080`, `16:9 → 1920×1080`). `maxCharsForWidth` also scales with canvas width so wrap-count adapts. New top-level `subAspect` const reads `body.aspectRatio` and threads through all 4 call sites.

### B) Hybrid Planner scene-image defaulted to posed/smiling portraits
`/api/hybrid/scene-image/route.ts:492` had `actionScene = body.isActionScene === true` — anti-static negatives fired ONLY when caller explicitly opted in. Hybrid Planner page.tsx is LOCKED (Henry's rule) so it never passes `isActionScene=true` → posed default → all-smiling all-facing-camera output.

**Fix:** Flipped the default. Now anti-static negatives fire UNLESS caller passes `isStillScene=true` (for explicit portrait/establishing shots). Also added cinematic action anchor to the positive prompt by default: *"Cinematic film still, movie frame, scene captured mid-action, characters in motion expressing emotion, NOT a posed portrait, NOT a studio shot, NOT smiling at camera, dynamic composition"*.

**Prevention rule (locked):** Any anti-default behavior that protects against a model prior MUST default to ON unless explicitly opted out. Opt-in defaults (`isActionScene === true`) only fire from callers who know about the flag — locked pages and 3rd-party callers get the broken default.

---

## P-2026-06-08 — Free Mode: PR #56 reduced image count per scene (regression)

**Symptom (verbatim, Henry):** "u have reduce the image generated by hybrid in freemode - i requested for more images in the scene not less".

**Root cause:** PR #56 fixed narration/subtitle/image sync by sizing each scene's image slot to its REAL narration audio duration (`scenePerSceneDuration = sceneNarrations.map(n => Math.max(2, Math.min(60, n.durationSec)))`). That fixed timing drift but **capped visual duration at the audio length** — short narration → short scene → fewer images (image count = `ceil(sceneDuration / secondsPerImage)`). User's `effectiveDuration` (their chosen total target length, e.g. 60s) was completely ignored.

Example: user picks 60s total, AI returns 5 scenes, each narration is ~5s. Pre-#56: each scene = 12s, 12 images/scene at 1s/image. Post-#56: each scene = 5s, 5 images/scene. **More than half the gallery vanished.**

**Fix applied:** Honor BOTH constraints — visual duration ≥ audio (no narration cut) AND scale up to user's effectiveDuration when audio is shorter.

```ts
const realDurations = sceneNarrations.map(n => Math.max(2, n.durationSec));
const realTotal = realDurations.reduce((a, b) => a + b, 0) || 1;
const scaleFactor = Math.max(1, effectiveDuration / realTotal);  // never < 1
const scenePerSceneDuration = realDurations.map(d => {
  const scaled = d * scaleFactor;
  return Math.max(d, Math.min(60, scaled));  // floor at audio length, cap 60s
});
```

User's effectiveDuration becomes a FLOOR on total length; if real audio exceeds it, we use real audio (no narration cut). Otherwise we scale up proportionally → more visual seconds per scene → more images per scene.

Sync invariants preserved: narrationList.startTime still cumulates VISUAL durations (so audio plays at scene visual boundaries with trailing silence if visual > audio). pacingEntries unchanged.

**Prevention rule (locked):** Any pipeline where user picks a target duration MUST honor that duration as a FLOOR. Sizing visuals to actual content length without scaling to the user's pick produces shorter-than-requested output and (when content has a per-unit budget like images-per-second) reduces the gallery size proportionally. Always scale up; never shrink below content.
