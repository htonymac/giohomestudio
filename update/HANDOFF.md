...
7. **Assembly too slow → ~2x+ faster** — `app/api/assembly/execute/route.ts`: ultrafast preset on intermediate clip encodes + concurrency 4→7 (8-core box) + final_merge `-c:v copy` when concat already covers duration (skip the full re-encode). 18-seg/63s: 42s→20s; audio path verified (h264+aac). PROBLEM_AND_FIX #47. HEAD `d4a455b`.
