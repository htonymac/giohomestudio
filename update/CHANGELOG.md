...
## 2026-07-03/04 — **Story pipeline restructure + mobile viewport (PRs #213–#215)**

**Story/draft stage (Henry: "60-min request → 40s nonsense; pasted 18-scene script → 2 min"):**
- **Verbatim authored-script path** (NEW `src/lib/story/authoredScript.ts`) — a pasted, already-written scene script (`**Scene N — Title, X–Y Minutes:**`, en-dash tolerant, false-positive guarded) is now detected and used AS-IS in BOTH children and hybrid planners: scene demarcation, per-scene text, and authored timings preserved; no LLM rewrite. Children per-scene safety scan kept (block → drop, soften → clean). 20/20 unit tests (`tests/test-authored-script.mts`).
- **The 40s bug** — children `expandContent` sent NO targetDuration (story-expand defaulted 120s) and kept only the 2-3 sentence summary with narrationText unset. Now sends the planner target and captures `fullScript` → narrationText.
- **scene-plan duration-aware** — was hardcoded "Aim for 5-10 scenes" and never read `targetDuration`. Now ~1 scene/40s (clamp 4-40), per-scene `durationEstimate` = share of budget, maxTokens + input slice scale. Accepts number seconds or legacy picker strings. Children now passes real total seconds (was `movieSceneDuration` "5s").
- **story-expand continuation passes scale** with the ask (~1 per 1200 words needed, cap 8; was hard-capped 2 → a 60-min/7800-word ask topped out ~40%).
- **Multi-topic builder** (NEW `src/lib/children/multiTopic.ts` + `buildMultiTopic` in children planner) — "60 MIN - SPELLING 2-5 LETTER 20 MIN - ALPHABET 10 MIN - PLAY 15 MIN - 3 BEDTIME STORIES" now builds ONE segmented video: deterministic card rounds per word length, LLM story per bedtime story (each demarcated), play-teaching segment, remainder time split. Typo-tolerant parse (17/17 tests on Henry's verbatim brief).

**Mobile (Henry: "app unusable on phone"):**
- Root cause: **no viewport meta anywhere** → phones assumed ~980px layout and shrink-to-fit; the ≤768px mobile shell (drawer sidebar, hamburger, grid collapse) already existed but could never fire. Fixed with one `export const viewport` in `app/layout.tsx` + `.gh-side-rail` stacking for the 4 fixed side rails (story-bank ×2, free-mode, commercial). Desktop untouched — all rules inside the media query.
