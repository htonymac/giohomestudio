...
function MoviePlannerInner() {
  ...
  const [selectedImageModelId, setSelectedImageModelId] = useState("segmind_pruna"); // Henry 2026-06-16 cost fix: off FAL (was fal_flux_schnell)
  ...
  return (
    <div style={{ background: ds.color.paper, minHeight: "100vh", padding: "0 0 60px", fontFamily: ds.font.sans }}>
      ...
      {/* ════════════════════════════════════════════════════════════════════ */}
      {/* ════════════════════════════════════════════════════════════════════ */}
      {/* SCENES TAB — extracted to tabs/ScenesTab.tsx (movie-planner Wave 2.2) */}
      {/* ════════════════════════════════════════════════════════════════════ */}
      {activeTab === "scenes" && (
        <ScenesTab
          cardStyle={cardStyle} labelStyle={labelStyle} inputStyle={inputStyle} btnPrimary={btnPrimary}
          s2={s2} border={border} muted={muted} accent={accent} blue={blue} green={green} gold={gold} red={red} purple={purple}
          badgeStyle={badgeStyle} methodColors={methodColors} costColors={costColors} SCENE_ENERGY_COLOR={SCENE_ENERGY_COLOR}
          continuousMotionEnabled={continuousMotionEnabled} setContinuousMotionEnabled={setContinuousMotionEnabled}
          cmError={cmError} setCmError={setCmError} cmStatus={cmStatus} setCmStatus={setCmStatus}
          cmFinalVideoUrl={cmFinalVideoUrl} setCmFinalVideoUrl={setCmFinalVideoUrl}
          cmTotalDuration={cmTotalDuration} setCmTotalDuration={setCmTotalDuration}
          cmSegmentDuration={cmSegmentDuration} setCmSegmentDuration={setCmSegmentDuration}
          cmProvider={cmProvider} setCmProvider={setCmProvider}
          cmRunning={cmRunning} startContinuousMotion={startContinuousMotion}
          totalScenes={totalScenes}
          scenes={scenes as unknown as import("./tabs/ScenesTab").ScenesTabScene[]}
          savedCharacters={savedCharacters as unknown as import("./tabs/ScenesTab").ScenesTabSavedChar[]}
          sceneImages={sceneImages} storyEra={storyEra} storyCulture={storyCulture}
          sceneViewMode={sceneViewMode} setSceneViewMode={setSceneViewMode}
          runningIntelligence={runningIntelligence} runSceneIntelligence={runSceneIntelligence}
          sceneIntelligence={sceneIntelligence}
          generatingAllImages={generatingAllImages} generatingSceneImage={generatingSceneImage}
          generateAllSceneImages={generateAllSceneImages}
          autoSfx={autoSfx} setActiveTab={setActiveTab}
          effectiveVideoModelId={effectiveVideoModelId} effectiveImageModelId={effectiveImageModelId}
          setAidMode={setAidMode} setShowAidPicker={setShowAidPicker}
          moviePlan={moviePlan as unknown as import("./tabs/ScenesTab").ScenesTabMoviePlan | null}
          renderScene={renderScene} saveProject={saveProject} renderingScene={renderingScene}
          sceneGenProgress={sceneGenProgress} sceneVideos={sceneVideos} sceneVideoVersions={sceneVideoVersions}
          sceneStyles={sceneStyles} setSceneStyles={setSceneStyles} effectiveProjectStyle={effectiveProjectStyle}
          makeSceneImage={makeSceneImage as unknown as (scene: import("./tabs/ScenesTab").ScenesTabScene) => void | Promise<void>}
          makeSceneVideo={makeSceneVideo as unknown as (scene: import("./tabs/ScenesTab").ScenesTabScene) => void | Promise<void>}
          generatingSceneVideos={generatingSceneVideos}
          updateScene={updateScene as unknown as (n: number, p: Partial<import("./tabs/ScenesTab").ScenesTabScene>) => void}
          expandedSceneId={expandedSceneId} setExpandedSceneId={setExpandedSceneId}
          handleDragStart={handleDragStart} handleDrop={handleDrop} dragSource={dragSource}
          moveScene={moveScene} duplicateScene={duplicateScene} deleteScene={deleteScene}
          projectId={projectId} setLastAction={setLastAction} setSceneImages={setSceneImages}
        />
      )}
      ...
      {activeTab === "sound" && (
        <SoundTab
          ...
          setCastVoiceMap={setCastVoiceMap}
          generatingPerLineVoices={generatingPerLineVoices}
          setGeneratingPerLineVoices={setGeneratingPerLineVoices}
          scenes={scenes}
          projectId={projectId}
          effectiveSoundTier={effectiveSoundTier}
          sceneDialogueAudio={sceneDialogueAudio}
          setSceneDialogueAudio={setSceneDialogueAudio}
          sceneVideos={sceneVideos}
          setSceneVideos={setSceneVideos}
          lipsyncingScenes={lipsyncingScenes}
          setLipsyncingScenes={setLipsyncingScenes}
          SOUND_TIERS_MOVIE={SOUND_TIERS_MOVIE}
          movieTierToGhsSoundTierId={movieTierToGhsSoundTierId}
          setSoundTier={setSoundTier}
          setModelSettings={setModelSettings}
          openTierInfo={openTierInfo}
          setOpenTierInfo={setOpenTierInfo}
          autoSfx={autoSfx}
          ...
          generateMovieMusic={generateMovieMusic}
          ...
          setSelectedMusicName={setSelectedMusicName}
          aiMusicPickLog={aiMusicPickLog}
          setAiMusicPickLog={setAiMusicPickLog}
          assetToMediaUrl={assetToMediaUrl}
          totalScenes={totalScenes}
          ...
          setNarrationTexts={setNarrationTexts}
      ...
      {/* ════════════════════════════════════════════════════════════════════ */}
      {/* ════════════════════════════════════════════════════════════════════ */}
      {/* ASSEMBLY TAB — extracted to tabs/AssemblyTab.tsx (movie-planner Wave 3) */}
      {/* ════════════════════════════════════════════════════════════════════ */}
      {activeTab === "assembly" && (
        <AssemblyTab
          cardStyle={cardStyle} labelStyle={labelStyle} inputStyle={inputStyle} btnPrimary={btnPrimary}
          s2={s2} surface={surface} border={border} muted={muted}
          accent={accent} green={green} gold={gold} red={red} purple={purple}
          ProgressBar={ProgressBar}
          savedCuts={savedCuts} setSavedCuts={setSavedCuts}
          showCutsPanel={showCutsPanel} setShowCutsPanel={setShowCutsPanel}
          assemblyName={assemblyName} setAssemblyName={setAssemblyName}
          assemblySelectedIds={assemblySelectedIds} setAssemblySelectedIds={setAssemblySelectedIds}
          assembledUrl={assembledUrl} setAssembledUrl={setAssembledUrl}
          setLastAction={setLastAction}
          preflightResult={preflightResult as unknown as import("./tabs/AssemblyTab").AssemblyTabPreflightResult | null}
          preflightRunning={preflightRunning} runPreflight={runPreflight}
          assemblyReadiness={assemblyReadiness}
          validation={validation} validating={validating} runValidation={runValidation}
          moviePlan={moviePlan as unknown as import("./tabs/AssemblyTab").AssemblyTabMoviePlan | null}
          totalScenes={totalScenes} generatedScenes={generatedScenes}
          scenes={scenes as unknown as import("./tabs/AssemblyTab").AssemblyTabScene[]}
          sceneVideos={sceneVideos} sceneImages={sceneImages}
          assemblyMediaPrefs={assemblyMediaPrefs} setAssemblyMediaPrefs={setAssemblyMediaPrefs}
          assembling={assembling} assemblyComplete={assemblyComplete}
          assemblyProgress={assemblyProgress}
          selectedMusicUrl={selectedMusicUrl}
          aiSupervisorReport={aiSupervisorReport} aiSupervisorRunning={aiSupervisorRunning} runAiSupervisor={runAiSupervisor}
          effectiveSubtitleConfig={effectiveSubtitleConfig}
          subtitleConfig={subtitleConfig} setSubtitleConfig={setSubtitleConfig}
          patchProjectSettings={patchProjectSettings}
          introUrl={introUrl} setIntroUrl={setIntroUrl}
          outroUrl={outroUrl} setOutroUrl={setOutroUrl}
          generatingIntro={generatingIntro} setGeneratingIntro={setGeneratingIntro}
          generatingOutro={generatingOutro} setGeneratingOutro={setGeneratingOutro}
          title={title}
          screenplayAuthor={screenplayAuthor} setScreenplayAuthor={setScreenplayAuthor}
          movieMadeBy={movieMadeBy} setMovieMadeBy={setMovieMadeBy}
          movieIdeaFrom={movieIdeaFrom} setMovieIdeaFrom={setMovieIdeaFrom}
          subtitleMatchResult={subtitleMatchResult} setSubtitleMatchResult={setSubtitleMatchResult}
          expandedStory={expandedStory} idea={idea}
          assembleMovie={assembleMovie} setActiveTab={setActiveTab}
        />
      )}
